...
class Request {
    constructor(input, init = {}) {
        ...
        // fetch spec options
        this.followAllRedirects = typeof init.followAllRedirects === 'undefined' ? true : init.followAllRedirects;
        this.jar = init.jar;
...
exportable.getNodeRequestOptions = function getNodeRequestOptions(request) {
    ...
    if (!headers.has('Cookie') && request.jar) {
        headers.set('Cookie', request.jar.getCookiesSync ? request.jar.getCookiesSync(request.url) : request.jar.getCookies(request.url))
    }

    // HTTP-network-or-cache fetch step 12
    if (!headers.has('User-Agent')) {
        headers.set('User-Agent', `better-electron-fetch/1.0 ${request.useElectronNet ? 'electron' : 'node'} (+https://github.com/elmthedev/better-electron-fetch)`)
    }
